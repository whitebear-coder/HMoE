...
class GPO_Model(nn.Module):   
    def __init__(self, encoder):
        super(GPO_Model, self).__init__()
        self.encoder = encoder
        self.gpool = GPO(8, 8)

        self.mvs = MultiView(768,256)

        unfreeze_layers = ['layer.8', 'layer.2', 'layer.11', 'layer.1']
    ...
    def forward(self, code_inputs=None, nl_inputs=None, code_prefix=None): 
        ...
        if code_inputs is not None:
            ...
            outputs = self.encoder(code_inputs,attention_mask=code_inputs.ne(1), code_prefix=code_prefix)[0]
